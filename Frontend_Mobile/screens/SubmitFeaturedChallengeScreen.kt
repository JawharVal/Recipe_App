...
@Composable
fun SubmitFeaturedChallengeScreen(navController: NavController) {
    val context = LocalContext.current

    // Initialize API and ViewModel for challenges
    ...
    var maxSubmissions by remember { mutableStateOf("") } // as String input
    val maxSubmissionsInt = maxSubmissions.toIntOrNull() ?: 1

    // Image state for the banner challenge
    ...
    val imagePickerLauncher = rememberLauncherForActivityResult(
        ...
        onResult = { pickedUri: Uri? ->
            if (pickedUri != null) {
                ...
                isUploading = true
                Log.d("SubmitFeaturedChallenge", "User picked image: $pickedUri")
                // Use your viewModel or other helper method to upload the image
                viewModel.uploadProfileImage(context, pickedUri) { uploadedUrl ->
                    isUploading = false
                    if (uploadedUrl != null) {
                        uploadedImageUrl = uploadedUrl
                        Log.d("SubmitFeaturedChallenge", "Successfully uploaded: $uploadedUrl")
                    } else {
                        Log.e("SubmitFeaturedChallenge", "Failed to upload image.")
                    }
                }
            }
        }
    ...
    Scaffold(
        topBar = { TopAppBar(title = { Text("Submit Featured Challenge") }) }
    ) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Button(
                ...
                Text(if (selectedImageUri == null) "Select Banner Image" else "Change Banner Image")
            }
            ...
            Spacer(modifier = Modifier.height(8.dp))
            // Deadline input (opens DatePicker on click)
            OutlinedTextField(
                value = deadline,
                onValueChange = {},
                label = { Text("Deadline") },
            ...
            Button(
                onClick = {
                    ...
                    // Create new challenge DTO. Mark it as featured.
                    val newChallenge = ChallengeDTO(
                        ...
                        featured = true // This ensures it's a featured challenge
                    )
                ...
                Text("Submit Featured Challenge")
            }
        }
    }
}
...
@Composable
fun ChallengeImages(imageUriString: String, isUploading: Boolean) {
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .height(180.dp)
